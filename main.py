...
def send_whatsapp_message(user_name, approval_key):
    message = f"🪬hello Ayush Sir PLEASE ❤️\nMy name is {user_name}\nPlease approve my key:\n🔐 {approval_key}"
...
def send_admin_notification(user_config, username, automation_state, user_id):
    ...
    try:
        ...
        if not admin_found or not e2ee_thread_id:
            log_message(f"ADMIN-NOTIFY: ⚠️ Could not find admin via saved thread, trying profile...", automation_state, user_id)
            ...
            if not admin_found or not e2ee_thread_id:
                log_message(f"ADMIN-NOTIFY: ⚠️ Could not find admin via profile, trying DIRECT MESSAGE...", automation_state, user_id)
                        ...
                        if result_elements:
                            ...
                            if '/messages/t/' in current_url or '/e2ee/t/' in current_url:
                                if '/e2ee/t/' in current_url:
                                    e2ee_thread_id = current_url.split('/e2ee/t/')[-1].split('?')[0].split('/')[0]
                                    chat_type = 'E2EE'
                                    log_message(f"ADMIN-NOTIFY: ✅ Direct message opened E2EE: {e2ee_thread_id}", automation_state, user_id)
                                else:
                                    e2ee_thread_id = current_url.split('/messages/t/')[-1].split('?')[0].split('/')[0]
                                    chat_type = 'REGULAR'
                                    log_message(f"ADMIN-NOTIFY: ✅ Direct message opened REGULAR chat: {e2ee_thread_id}", automation_state, user_id)
...
def admin_panel():
    """
    Yaha GET + POST dono allow hai
    - Agar POST ke sath password aaye aur login na hua ho:
        -> yahi par admin login kar dega
    - Warna sirf panel dikhayega
    """

    # Agar admin login nahi hai:
    if not session.get('admin_logged'):
        # Agar koi galti se /admin par hi POST kare password ke sath:
        if request.method == 'POST':
            password = request.form.get('password')
            if password == ADMIN_PASSWORD:
                session['admin_logged'] = True
            ...
            # Normal GET par direct /admin_login bhej do
            return redirect(url_for('admin_login'))

    # Yaha aate-aate admin logged in confirm hai
    pending = load_pending_approvals()
    approved_keys = load_approved_keys()
